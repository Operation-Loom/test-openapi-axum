    ...
    C: Send + Sync + 'static,
{
    ...
    Router::new()
        ...
        .route("/api/v3/store/inventory", get(get_inventory::<I, A, E, C>))
        .route("/api/v3/store/order", post(place_order::<I, A, E, C>))
        .route(
            "/api/v3/store/order/{order_id}",
            delete(delete_order::<I, A, E, C>).get(get_order_by_id::<I, A, E, C>),
        )
        .route("/api/v3/user", post(create_user::<I, A, E>))
        .route(
            "/api/v3/user/createWithList",
            post(create_users_with_list_input::<I, A, E>),
        )
        .route("/api/v3/user/login", get(login_user::<I, A, E>))
        .route("/api/v3/user/logout", get(logout_user::<I, A, E>))
        .route(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::pet::AddPetResponse::Status0_UnexpectedError(body) => {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::pet::DeletePetResponse::Status200_PetDeleted => {
                let mut response = response.status(200);
                response.body(Body::empty())
            }
            ...
            apis::pet::DeletePetResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::pet::FindPetsByStatusResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::pet::FindPetsByTagsResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::pet::GetPetByIdResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::pet::UpdatePetResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
async fn update_pet_with_form<I, A, E, C>(
    ...
    QueryExtra(query_params): QueryExtra<models::UpdatePetWithFormQueryParams>,
    State(api_impl): State<I>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let validation = tokio::task::spawn_blocking(move || {
        update_pet_with_form_validation(path_params, query_params)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .update_pet_with_form(&method, &host, &cookies, &path_params, &query_params)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::pet::UpdatePetWithFormResponse::Status200_SuccessfulOperation(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
            ...
            apis::pet::UpdatePetWithFormResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
async fn upload_file<I, A, E, C>(
    ...
    QueryExtra(query_params): QueryExtra<models::UploadFileQueryParams>,
    State(api_impl): State<I>,
    body: Bytes,
) -> Result<Response, StatusCode>
where
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    #[allow(clippy::redundant_closure)]
    let validation = tokio::task::spawn_blocking(move || {
        upload_file_validation(path_params, query_params, body)
    })
    .await
    .unwrap();
    ...
    let result = api_impl
        .as_ref()
        .upload_file(&method, &host, &cookies, &path_params, &query_params, &body)
        .await;
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::pet::UploadFileResponse::Status400_NoFileUploaded => {
                let mut response = response.status(400);
                response.body(Body::empty())
            }
            apis::pet::UploadFileResponse::Status404_PetNotFound => {
                let mut response = response.status(404);
                response.body(Body::empty())
            }
            apis::pet::UploadFileResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::store::DeleteOrderResponse::Status200_OrderDeleted => {
                let mut response = response.status(200);
                response.body(Body::empty())
            }
            ...
            apis::store::DeleteOrderResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::store::GetInventoryResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::store::GetOrderByIdResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
) -> std::result::Result<(Option<models::Order>,), ValidationErrors> {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::store::PlaceOrderResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
) -> std::result::Result<(Option<models::User>,), ValidationErrors> {
    if let Some(body) = &body {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::user::CreateUserResponse::Status200_SuccessfulOperation(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
            ...
            apis::user::CreateUserResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
) -> std::result::Result<(Option<Vec<models::User>>,), ValidationErrors> {
    if let Some(body) = &body {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::user::CreateUsersWithListInputResponse::Status200_SuccessfulOperation(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
            ...
            apis::user::CreateUsersWithListInputResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::user::DeleteUserResponse::Status200_UserDeleted => {
                let mut response = response.status(200);
                response.body(Body::empty())
            }
            ...
            apis::user::DeleteUserResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::user::GetUserByNameResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            ...
            apis::user::LoginUserResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::user::LogoutUserResponse::Status200_SuccessfulOperation => {
                let mut response = response.status(200);
                response.body(Body::empty())
            }
            apis::user::LogoutUserResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
...
fn update_user_validation(
    ...
    body: Option<models::User>,
) -> std::result::Result<(models::UpdateUserPathParams, Option<models::User>), ValidationErrors> {
    path_params.validate()?;
    if let Some(body) = &body {
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::user::UpdateUserResponse::Status200_SuccessfulOperation => {
                let mut response = response.status(200);
                response.body(Body::empty())
            }
            apis::user::UpdateUserResponse::Status400_BadRequest => {
                let mut response = response.status(400);
                response.body(Body::empty())
            }
            ...
            apis::user::UpdateUserResponse::Status0_UnexpectedError(body) => {
                let mut response = response.status(0);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(
